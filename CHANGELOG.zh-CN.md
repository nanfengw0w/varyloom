...
已发布到 [npm](https://www.npmjs.com/package/varyloom/v/0.1.1)。对应源码标签：[`v0.1.1`](https://github.com/nanfengw0w/varyloom/releases/tag/v0.1.1)。

### 修复

- 初始化失败时释放控制器资源，并回收取消之后才完成加载的自有图片资源。
- 控制器销毁后，尚未完成的初始化不会重新启动渲染；自定义效果的销毁方法抛错时，仍继续清理其余资源。
